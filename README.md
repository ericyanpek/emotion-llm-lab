...
本项目以 Kiro / Claude Code agent 多角色并行的方式推进：

| Role | 职责 | 所有权路径 |
|---|---|---|
| **A · Builder** | 训练配置 + 基础设施 + persona / 数据 schema | `infrastructure/` `configs/sft_*` `configs/dpo_*` `data/sft/` `data/dpo/` `personas/` `schemas/` `scripts/run-*.sh` |
| **B · Verifier** | Eval pipeline + LLM-as-judge + drift probes | `configs/eval/` `data/eval/` `scripts/eval/` `scripts/eval_persona.py` |
| **C · Documenter** | Learning plan + 理论回补 | `docs/learning/` |
| **D · Analyst** | 训练/eval 结果分析（**预留**，真 v1 训练有数据时激活） | `docs/experiments/` |

完整路径分区表、共享文件 owner 规则、handoff 模板见 [`docs/collaboration.md`](./docs/collaboration.md)。通用协作规则见用户级 steering：

```
~/.kiro/steering/multi-agent-collaboration.md    # 四角色 + 四规则
~/.kiro/steering/adr-conventions.md              # ADR 编号约定（跨项目通用）
```
